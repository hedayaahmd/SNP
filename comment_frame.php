...
<head>
	<title></title>
     <link rel="stylesheet" href="assets/css/reg_styling.css">
</head>
...
<body>
	...
	if(isset($_POST['postComment' . $post_id])) {
		...
		$insert_post = mysqli_query($connection, "INSERT INTO coments VALUES ('', '$post_body', '$userLoggedIn', '$posted_to', '$date_time_now', 'no', '$post_id')");
    ...
    <?php
    $comments_query=mysqli_query($connection,"SELECT * FROM coments where post_id='$post_id' ORDER BY id ASC");
    ...
    if($count != 0){
        while($comment=mysqli_fetch_array($comments_query)){
            ...
            //timeFrame
            $date_time_now=date("Y-m-d H:i:s");
            $start_date =new DateTime($date_added);//post date time
